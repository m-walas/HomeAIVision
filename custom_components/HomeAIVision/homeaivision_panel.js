...
class SettingsCard extends nt {
  ...
  async fetchConfigData() {
    try {
      const data = await this.hass.callApi("GET", "/api/homeaivision/config");

      if (response.ok && data.success) {
        this.configData = data.config;
        this.requestUpdate();
        console.log("[HomeAIVision] Successfully fetched config data");
      } else {
        console.warn("[HomeAIVision] Failed to fetch config data", data.error);
      }
    } catch (error) {
      console.error(`[HomeAIVision] Error fetching config data: ${error}`);
    }
  }
  ...
  renderConfigData() {
    ...
    return html`
      <div class="config-data">
        ...
        <ul>
          <li><strong>Integration Title:</strong> ${this.configData.integration_title}</li>
          <li><strong>Azure API Key:</strong> ${this.configData.azure_api_key}</li>
          <li><strong>Azure Endpoint:</strong> ${this.configData.azure_endpoint}</li>
          <li><strong>Camera URL:</strong> ${this.configData.cam_url}</li>
          <li><strong>Time Between Requests:</strong> ${this.configData.time_between_requests} seconds</li>
          <li><strong>Detected Object:</strong> ${this.configData.detected_object}</li>
          <li><strong>Confidence Threshold:</strong> ${this.configData.confidence_threshold}</li>
          <li><strong>Send Notifications:</strong> ${this.configData.send_notifications ? "Yes" : "No"}</li>
          <li><strong>Notification Language:</strong> ${this.configData.notification_language}</li>
          <li><strong>Organize By Day:</strong> ${this.configData.organize_by_day ? "Yes" : "No"}</li>
          <li><strong>Days to Keep Images:</strong> ${this.configData.days_to_keep}</li>
          <li><strong>Max Images:</strong> ${this.configData.max_images}</li>
        </ul>
      </div>
    `;
  }
  ...
  static get styles() {
    return o`
      ...
      .config-data {
        font-family: var(--paper-font-body1-font-family, inherit);
      }
      .config-data ul {
...
class HomeAIVisionPanel extends nt {
  ...
  static version = "v2024.9.7"; // Placeholder for panel version
  ...
  async loadImages() {
    try {
      const data = await this.hass.callApi("GET", "/api/homeaivision/images");
...
if (!customElements.get("homeaivision-panel")) {
